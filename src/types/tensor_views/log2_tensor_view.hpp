...
namespace happyml {
    class Log2TensorView : public BaseTensorUnaryOperatorView {
    public:
        explicit Log2TensorView(const std::shared_ptr<BaseTensor> &tensor) : BaseTensorUnaryOperatorView(tensor) {
        }

        void printMaterializationPlan() override {
            cout << "Log2TensorView{" << rowCount() << "," << columnCount() << "," << channelCount() << "}->";
