...
namespace happyml {
    class TensorValueTransform3View : public happyml::BaseTensorUnaryOperatorView {
    public:
        TensorValueTransform3View(const shared_ptr<BaseTensor> &tensor,
        ...
        void printMaterializationPlan() override {
            cout << "TensorValueTransform3View{" << rowCount() << "," << columnCount() << "," << channelCount()
                 << "}->";
            child->printMaterializationPlan();
        }

        float getValue(size_t row, size_t column, size_t channel) override {
            return transformFunction(child->getValue(row, column, channel), constant_);
        }
