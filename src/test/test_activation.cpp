...
void testTanh() {
    auto valueRange = columnVector(
            {-100.f, -10.f, -1.f, -0.75, -0.5, -0.25f, 0.f, 0.25f, 0.5f, 0.75f, 1.f, 10.f, 100.f});
    auto tanhActivationFunction = make_shared<TanhActivationFunction>();
    auto result = tanhActivationFunction->activate(valueRange);
    auto expected = columnVector(
            {-1.000, -1.000, -0.761594, -0.635149, -0.462, -0.245, 0.000, 0.245, 0.462, 0.635149, 0.761594, 1.000,
             1.000});
...
void testTanhDerivative() {
    ...
    auto expected = columnVector(
            {0.000, 0.000, 0.420, 0.596586, 0.786448, 0.940, 1.000, 0.940, 0.786448, 0.596586, 0.420, 0.000, 0.000});
    assertEqual(expected, result);
}
