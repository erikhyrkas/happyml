...
namespace happyml {
    ...
    class Optimizer {
    public:
        virtual shared_ptr<NeuralNetworkFunction> createConvolutional2d(const string &label,
                                                                        vector<size_t> input_shape,
                                                                        size_t filters,
                                                                        size_t kernel_size,
                                                                        uint8_t bits) = 0;

        virtual shared_ptr<NeuralNetworkFunction> createFullyConnectedNeurons(const string &label,
                                                                              size_t input_size,
                                                                              size_t output_size,
                                                                              uint8_t bits) = 0;

        virtual shared_ptr<NeuralNetworkFunction> createBias(const string &label,
                                                             vector<size_t> input_shape,
